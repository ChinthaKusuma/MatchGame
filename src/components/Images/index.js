...
class Images extends Component {
  ...
  selectImage1 = id => {
    const one = imagesList[Math.ceil(Math.random() * imagesList.length)]
    const {image, count} = this.state
    if (image.id === id) {
      this.setState({
        count: count + 1,
      })
  ...
  render() {
    ...
    const list2 = list1.filter(each => each.category === tab)
    const RandomItem = list1.find(each => each.id === image.id)
      ...
      <div className="bgContainer">
            ...
            <ul className="row1">
              ...
              <li>
                <div className="Score_Container">
                  <p className="para">
                    Score: <span className="span1">{count}</span>
                  </p>
        ...
        {gameover === false ? (
          <div>
            <div className="one12">
              <div>
                <img src={imageUrl} alt="match" className="imgRandom" />
              </div>
              <ul className="ul2">
                {tabsList.map(each => (
                  <TabItem
                    ...
                    key={each.tabId}
                    isActive={each.tabId === isActiveId}
                  />
                ))}
              </ul>
              ...
              <ul className="ul1">
                {list2.map(each => (
                  <ImagesItem
                    details={each}
                    selectImage1={this.selectImage1}
                    key={each.id}
                  />
                ))}
              </ul>
          ...
          <div className="two">
            <ScoreCard count={count} startGame1={this.startGame1} />
          </div>
        )}
      </div>
    )
